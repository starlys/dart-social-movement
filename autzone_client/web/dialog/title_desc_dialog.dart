...
class TitleDescDialog extends DialogBox {
  ...
  TitleDescDialog(this._topPrompt, this._descPrompt, this._titleEditable)
      : super() {}
  ...
  Future build() async {
    ...
    InputElement titleInput = form.addInput(
        'Title',
        typicalControlWidth(),
        Globals.maxTitleLength,
        title); //lengths not actually limited by databse, so these are fairly long
    if (!_titleEditable) titleInput.readOnly = true;
    TextAreaElement descInput = form.addTextArea(
        _descPrompt, typicalControlWidth(), 80, 10000, description);
