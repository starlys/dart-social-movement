/*
 * How did this file get here? Glitch in editor, cat on keyboard?
 * Keeping in case any edits were made here that aren't in the real copy
 *
 import 'dart:async';
import 'dart:math';
...
class Servant {
  ...
  Future<ConvGetResponse> convGet(ConvGetRequest args) async {
    ...
    await Database.safely('ConvGet', r.base, (db) async {
      ...
      Row convRow = await db.query('select project_id,event_id,title,open,from_conv_id,open,post_max_size,xuser_daily_max,last_activity,delete_time from conv where id=${args.convId}').single;
      ...
      DateTime earliestTime = new DateTime(1970);
      ...
      if (postRows != null) {
        for (Row postRow in postRows) {
          ...
          p.avatarUrl = ImageLib.getAvatarUrl(p.authorId, postRow.avatar_no);
  ...
  Future<ConvGetRulesResponse> convGetRules(ConvGetRulesRequest args) async {
    ...
    await Database.safely('ConvGetRules', r.base, (db) async {
      //get from conv
      Row convRow = await db.query('select project_id,event_id,post_max_size,xuser_daily_max from conv where id=${args.convId}').single;
      int projectId = convRow.project_id; //null ok
      int eventId = convRow.event_id; //null ok
  ...
  Future<APIResponseBase> convSave(ConvSaveRequest args) async {
    ...
    await Database.safely('ConvSave', r, (db) async {
      bool isNew = args.convId == 0;

      //update existing conv
      if (!isNew) {
        //overwrite proj/event keys from existing conv (they cant be changed)
        Row convRow = await db.query('select project_id,event_id from conv where id=${args.convId}').single;
        ...
        if (args.openingPostId != null) {
          ...
          if (templatePost.length > 0) {
            ...
            String hasImage = templatePost[0].has_image;
            int twPosition = templatePost[0].tw_position;
            ConvLib.writeConvPost(db, convId, ai.id, ptext, twPosition, hasImage == 'Y');
          }
        }
      }
    });
  ...
  Future<ConvPostGetResponse> convPostGet(ConvPostGetRequest args) async {
    ...
    await Database.safely('ConvPostGet', r.base, (db) async {
      ...
      Row postRow = await db.query('select conv_id,author_id from conv_post where id=@i', {'i': args.postId}).single;
      int convId = postRow.conv_id;
      int authorId = postRow.author_id;

      //get info from the author
      Row authorRow = await db.query('select avatar_no from xuser where id=${authorId}').single;
      r.avatarUrl = ImageLib.getAvatarUrl(authorId, authorRow.avatar_no);

      //get info from the conv
      Row convRow = await db.query('select project_id,event_id,post_max_size,xuser_daily_max from conv where id=${convId}').single;
      int projectId = convRow.project_id; //null ok
      int eventId = convRow.event_id; //null ok
      ...
      if (projectId != null) {
        Row projectUserRow = await db.query('select spam_count from project_xuser where project_id=${projectId} and xuser_id=${authorId}').single;
  ...
  Future<APIResponseBase> convPostSave(ConvPostSaveRequest args) async {
    ...
    await Database.safely('ConvPostSave', r, (db) async {
      ...
      //if storing new post..
        ...
        //check permissions
        Permissions.checkConvPostPermissions(ApiGlobals.config, db, ai.id, args.convId, convRow, projectId, args.ptext.length);
        ...
        await ConvLib.writeConvPost(db, args.convId, authorId, ptextWithWarning, twPosition, false);
  ...
  Future<APIResponseBase> convPostImageSave(ConvPostImageSaveRequest args) async {
    ...
    await Database.safely('ConvPostImageSave', r, (db) async {
      ...
      //write it
      String postId = await ConvLib.writeConvPost(db, args.convId, ai.id, args.ptext, null, true);
  ...
  Future<APIResponseBase> convSetReadPosition(ConvSetReadPositionRequest args) async {
    ...
    await Database.safely('ConvSetReadPosition', r, (db) async {
      //save it, only if user is joined
      await db.execute('update conv_xuser set position_flag=\'N\', read_position=@t where conv_id=${args.convId} and xuser_id=${ai.id} and status=\'J\'',
        {'t': WLib.wireToDateTime(args.positionWDT)});
    });
    return r;
  }
  ...
  Future<DocQueryResponse> docQuery(DocQueryRequest args) async {
    ...
    await Database.safely('DocQuery', r.base, (db) async {
      ...
      await for (Row row in db.query('select id,title from doc where project_id is null order by title')) {
  ...
  Future<DocGetResponse> docGet(DocGetRequest args) async {
    ...
    await Database.safely('DocGet', r.base, (db) async {
      ...
      Row docRow = await db.query('select id,title,project_id,revision_no from doc where ${whereClause}').single;
      ...
      if (r.projectId != null) {
        ...
        Row projRow = await db.query('select title from project where id=${r.projectId}').single;
  ...
  Future<APIResponseBase> docRollback(DocRollbackRequest args) async {
    ...
    await Database.safely('DocRollback', r, (db) async {
      //rights checks
      Row docRow = await db.query('select project_id,revision_no,title from doc where id=${args.docId}').single;
  ...
  Future<ProjectGetResponse> projectGet(ProjectGetRequest args) async {
    ...
    await Database.safely('ProjectGet', r.base, (db) async {
      //load project info
      Row projectRow = await db.query('select active,leadership,privacy,title,description,category_id from project where id=${args.projectId}').single;
      ...
      r.proposals = await db.query('select id,active,title,created_at from proposal where project_id=${args.projectId} order by created_at desc')
        ...
        ..active = row.active
        ..title = row.title
        ..createdAt = DateLib.formatDateTime(row.createdAt, tzName)
        ).toList();
  ...
  Future<ProjectUserQueryResponse> projectUserQuery(ProjectUserQueryRequest args) async {
    ...
    await Database.safely('ProjectUserQuery', r.base, (db) async {
      //get project info
      Row projectRow = await db.query('select privacy,leadership from project where id=${args.projectId}').single;
      ...
      String sql = 'select xuser_id,kind,nick,avatar_no,spam_count,'
        ...
        ' where project_id=${args.projectId} and (public_name like @pn or nick like @n)'
      ...
      await for (Row row in db.query(sql)) {
        ...
        u.avatarUrl = ImageLib.getAvatarUrl(u.userId, row.avatarNo);
  ...
  Future<ProposalGetResponse> proposalGet(ProposalGetRequest args) async {
    ...
    await Database.safely('ProposalGet', r.base, (db) async {
      //get row
      Row row = await db.query('select * from proposal where id=${args.proposalId}').single;
  ...
  Future<APIResponseBase> proposalSave(ProposalSaveRequest args) async {
    ...
    await Database.safely('ProposalSave', r, (db) async {
      ...
      if (args.kind == 'PROJ') {
        ...
        await ProposalLib.proposeInProject(db, ai.id, args.projectId, args.eligible, args.title,
          args.summary, args.options, args.days);
      }
      else if (args.kind == 'SYS') {
        await ProposalLib.proposeSystemChange(ApiGlobals.config, db, ai.id, args.title, args.summary);
      }
    });
    return r;
  }
  ...
  Future<APIResponseBase> proposalDelete(ProposalGetRequest args) async {
    ...
    await Database.safely('ProposalDelete', r, (db) async {
      ...
      Row row = await db.query('select created_by,kind from proposal where id=${args.proposalId}').single;
  ...
  Future<APIResponseBase> proposalUserSave(ProposalUserSaveRequest args) async {
    ...
    await Database.safely('ProposalUserSave', r, (db) async {
      //check eligibility
      Row proposalRow = await db.query('select project_id,eligible from proposal where id=${args.proposalId}').single;
  ...
  Future<PushQueueGetResponse> pushQueueGet(PushQueueGetRequest args) async {
    ...
    await Database.safely('PushQueueGet', r.base, (db) async {
      ...
      //load all unreads
      List<Row> convRows = await db.query('select conv.id as c_id,conv.title as c_title, project_xuser.kind as p_kind, event.created_by as e_created_by'
        ...
        ' where conv_xuser.position_flag=\'U\' and conv_xuser.xuser_id=${ai.id}').toList();
      ...
      if (isFull) {
        ...
        for (Row row in suggestRows.where((r) => r.status == 'I')) //invited
          copy1(row, 'I');
        for (Row row in suggestRows.where((r) => r.status == 'I')) //recommended
          copy1(row, 'R');
  ...
  Future<ResourceGetResponse> resourceGet(ResourceGetRequest args) async {
    ...
    await Database.safely('ResourceGet', r.base, (db) async {
      ...
      Row resourceRow = await db.query(sql).single;
  ...
  Future<UserQueryResponse> userQuery(UserQueryRequest args) async {
    ...
    await Database.safely('UserQuery', r.base, (db) async {
      ...
      if (args.name != null && args.name.length > 0) {
        String param1 = '%${args.name}%';
        whereClause = 'where id like @id or public_name like @name';
        params['id'] = param1;
        params['name'] = param1;
      }
      r.users = new List<UserQueryItem>();
      await for (Row row in db.query('select id,nick,kind,public_name,avatar_no from xuser ${whereClause} order by last_activity desc limit 100')) {
  ...
  Future<UserGetResponse> userGet(UserGetRequest args) async {
    ...
    await Database.safely('UserGet', r.base, (db) async {
      ...
      //load user
      if (doLoad) {
        Row userRow = await db.query('select nick,email,kind,site_admin,public_name,pref_email_notify,public_links,timezone,avatar_no from xuser where id=${args.userId}').single;
        r..nick = userRow.nick
          ..email = userRow.email
          ..kind = userRow.kind
      ...
      //hide some things if for another user
      if (!aboutMe) {
        r.prefEmailNotify = 'N';
        r.email = '';
      }
  ...
  Future<APIResponseBase> userSave(UserSaveRequest args) async {
    ...
    await Database.safely('UserSave', r, (db) async {
      ...
      if (args.email != null && args.email.length > 0) {
        ...
        var proposedEmail = {'email': args.email, 'code': code};
        ...
        String body = 'Automated message from ${settings['siteName']}'
          ...
          '\r\nthe email address.'
          '\r\n\r\n${link}';
        MailLib.send(ApiGlobals.config, args.email, 'Verify email address', body);
      }
  ...
  Future<APIResponseBase> userAvatarSave(UserAvatarSaveRequest args) async {
    ...
    await Database.safely('UserAvatarSave', r, (db) async {
      ImageLib.saveAvatar(db, ai.id, args.imageBytes);
    });
    return r;
  }
  ...
  Future<APIResponseBase> userRecoverPassword(UserRecoverPasswordRequest args) async {
    ...
    await Database.safely('UserRecoverPassword', r, (db) async {
      ...
      Row userRow = await db.query('select id,recovery_code,email,proposed_email from xuser where nick=@n and status=\'A\'',
        {'n': args.recoveryNick}).single;
      int userId = userRow.user_id;
      ...
      if (args.mode == 'E') {
        ...
        MailLib.send(ApiGlobals.config, emailTo, 'Recovery code', body);
      }
