  ...
  Future build() async {
    ...
    if (isNewEvent) {
      ...
      _event = EventGetResponse(
          startTimeU: WLib.packDateEntry(nowLocal.year, nowLocal.month,
              nowLocal.day, nowLocal.hour, nowLocal.minute));
    }

    //main content
    FormBuilder form = new FormBuilder(frame, 'Event');
    InputElement titleInput = form.addInput(
        'Title', typicalControlWidth(), Globals.maxTitleLength, _event.title);
    ...
    InputElement durationInput = form.addInput('Duration',
        typicalControlWidth(), Globals.maxTitleLength, _event.duration);
    ButtonElement mapButton = new ButtonElement()
      ..text = isNewEvent ? 'Set Location On Map' : 'Change Map Location';
    form.addAny('Map location', mapButton);
    TextAreaElement locationInput = form.addTextArea(
        'Location/directions',
        typicalControlWidth(),
        90,
        Globals.maxDescriptionLength,
    ...
    bar.addButton('OK', (e) async {
      ...
      EventSaveRequest saveArgs = new EventSaveRequest(
          ...
          startTime: WLib.packDateEntry(startsAt.year, startsAt.month,
      ...
      if (response.isOK) {
        if (isNewEvent)
          Messages.criticalMessage(
              'The event will be reviewed and posted later');
        hide(true);
      }
    });
