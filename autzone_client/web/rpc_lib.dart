...
class RpcLib {
  ...
  ///show a message only if the given object has an error state; argument should be APIResponseBase
  static void _handleAPIError(dynamic r) {
    if (r.isOK) return;
    Messages.timed(r.errorMessage ?? r.errorCode);
  }
  ...
  static Future<PushQueueGetResponse> pushQueueGet(PushQueueGetRequest req) async {
    ...
    //skipping showing errors here because they are not relevant to the user
    return response;
  }
