...
class UserDialog extends DialogBox {
  ...
  Future build() async {
    ...
    if (isNewUser)
      userAgreement = await RpcLib.docGet(new DocGetRequest(specialCode: 'UA'));
    ...
    if (!isNewUser)
      ...
      form.addExplanation(nickInput,
          'Your nickname (screen name) is shown to all users; use 5 or more characters.');
    InputElement pw1Input = form.addPasswordInput(
        isNewUser ? 'Password' : 'Change password', 200, 40);
    if (isNewUser)
      form.addExplanation(pw1Input, 'Passwords must be 5 or more characters');
    InputElement pw2Input = form.addPasswordInput('Repeat password', 200, 40);
    InputElement emailInput =
        form.addInput('Private email address', 200, 200, user.email);
    form.addExplanation(emailInput,
        'Optional email, but if you omit this, then there is no other way to recover a lost password.');
    ...
    if (isNewUser)
      form.addExplanation(
          kindInput, 'Select your affiliation with the movement.');
    FormBuilder publicSubform = form.addSubformFrame(
        'About Me (optional public information shown to all users)');
    ...
    InputElement publicNameInput =
        publicSubform.addInput('Real name', 200, 100, user.publicName);
    int nLinksCreated = 0;
    ...
    if (isNewUser) {
      ...
      DivElement docDiv = new DivElement()
        ..style.overflowY = 'scroll'
        ..style.maxHeight = '350px';
      newUserSubform.parent.append(docDiv);
      String userAgreementHtml =
          MarkdownGetSanitizingConverter().makeHtml(userAgreement.body);
    ...
    bool isValid() {
      ...
      if (isNewUser && !iAgreeChk.checked) {
      ...
      if (links == null) {
        form.showError(
            'You must select a kind of link for each item in About Me.');
        return false;
      }
      List<String> linkKinds = links.map((v) => v.name).toList();
      if (hasDuplicates(linkKinds)) {
    ...
    bar.addButton(isNewUser ? 'Create Account' : 'Save', (e) async {
      ...
      links.forEach((st) {
        linksMap[st.name] = st.value;
      });

      String savePW = trimInput(pw1Input);
      if (savePW.length == 0)
        savePW = null; //required by server to avoid updating
      ...
      if (response.isOK) {
        ...
        if (isNewUser) {
          ...
          Messages.criticalMessage(
              'Account created! Click "Log in" in the upper right corner to log in to your new account.');
        }
        hide(true);
        onChangeAccount();
      }
    ...
    if (!isNewUser) {
      bar.addButton('Delete Account', (e) async {
        StringDialog confirm = new StringDialog(
            'Type the word "delete" to delete your account', '', 6);
