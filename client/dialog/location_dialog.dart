...
class LocationDialog extends DialogBox {
  ...
  static Element _mapDiv; //this gets moved between the cubbyhole and the dialog (see notes in staticInit)
  ...
  @override int dialogHeight() {return 500;}
  ...
  Future build() async {
    ...
    TextInputElement streetInp = new TextInputElement() ..maxLength = 400 ..style.width = '170px';
    streetDiv.append(streetInp);
    ButtonElement streetBtn = new ButtonElement() ..text = 'Lookup Address or Zip/Postal code';
    streetDiv.append(streetBtn);
    frame.append(streetDiv);
    DivElement mapBorrower = new DivElement();
    frame.append(mapBorrower);
    frame.appendText('You can drag the map to scroll, zoom in or out, then click on the exact location to set the marker.');
    ...
    streetBtn.onClick.listen((e) {
      ...
      _geocoder.callMethod('geocode',[geoOptions, (results, status) {
    ...
    _mapClickSubscription = _googleMaps['event'].callMethod('addListener', [_map, 'click', (e) {
      var loc = e['latLng'];
  ...
  @override void hide(dynamic value) {
    //remove marker
    if (_marker != null) {
