...
class ProposalLib {
  ...
  static Future<bool> isEligibleToVote(PostgreSQLConnection db, int userId, bool isSiteAdmin, Map<String, dynamic> proposalRow) async {
    ...
    if (eligibleCode == 'E') return true; //everyone
      ...
      String pkind = await MiscLib.queryScalar(db, 'select kind from project_xuser where project_id=${projectId} and xuser_id=${userId}', null);
  ...
  static Future vote(PostgreSQLConnection db, int proposalId, int userId, int optionNo) async {
    ...
    final proposalRow = await MiscLib.queryRow(db, 'select timeout,active,timeout_action,pass_target,fail_target from proposal where id=${proposalId}', null);
    if (proposalRow == null) {
    ...
    if (proposalRow['timeout_action'] == 'F') {
      if (optionNo == 0 && proposalRow['pass_target'] != null) {
        ...
        int count = await MiscLib.queryScalar(db, 'select count(*) from proposal_xuser where proposal_id=${proposalId} and vote=0', null);
      ...
      if (optionNo == 1 && proposalRow['fail_target'] != null) {
        ...
        int count = await MiscLib.queryScalar(db, 'select count(*) from proposal_xuser where proposal_id=${proposalId} and vote=1', null);
  ...
  static Future closeProposal(PostgreSQLConnection db, int proposalId) async {
    //tally votes
    final sumRows = await MiscLib.query(db, 'select vote,count(vote) as c from proposal_xuser where proposal_id=${proposalId} group by vote', null);
    Map<String, int> voteCounts = new Map<String, int>(); //counts indexed by (option#.tostring)
    for (final sumRow in sumRows) {
      String option = sumRow['vote'].toString();
      int count = sumRow['c'];
    ...
    final proposalRow = await MiscLib.queryRowChecked(db, 'select active,kind,title,pass_target,fail_target,timeout_action,timeout,table_name,options,col_values,key_prefix,project_id,doc_id,created_by,dtext from proposal where id=${proposalId}',
      'Proposal does not exist', null);
  ...
  static Future<String> _allowNewRecord(PostgreSQLConnection db, String tableName, Map<String, dynamic> colValues) async {
    ...
    if (tableName == 'proposal' && colValues['kind'] == 'ROOT') {
      ...
      final existingProposalRow = await MiscLib.queryRow(db, 'select id from proposal where kind=\'ROOT\' and active=\'Y\' and doc_id=${docId}', null);
  ...
  static Future _finalizeKindJoin(PostgreSQLConnection db, int proposalId, Map<String, dynamic> proposalRow, int winOption) async {
    //join conversation(s)
    int projectId = proposalRow['project_id'];
    int createdBy = proposalRow['created_by']; //a user id
    int numJoined = 0;
    if (winOption == 0 && projectId != null) {
      final convUserRows = await MiscLib.query(db, 'select conv_xuser.conv_id,conv_xuser.status from conv_xuser inner join conv on conv_xuser.conv_id=conv.id where conv.project_id=${projectId} and conv_xuser.xuser_id=${createdBy}', null);
      for (final convUserRow in convUserRows) {
  ...
  static Future _finalizeKindSys(PostgreSQLConnection db, Map<String, dynamic> proposalRow, int winOption) async {
  ...
  static Future _finalizeKindProj(PostgreSQLConnection db, int proposalId, Map<String, dynamic> proposalRow, int winOption) async {
    ...
    int projectId = proposalRow['project_id'];
    final projUserRows = await MiscLib.query(db, 'select xuser_id from project_xuser where project_id=${projectId} and kind in (\'V\',\'A\',\'M\')', null);
    for (final projUserRow in projUserRows) {
    ...
    await db.execute('update proposal set delete_time=@d where id=${proposalId}',
      substitutionValues: {'d': WLib.utcNow().add(new Duration(days:180))});
  }

  ///perform follow up actions for closed proposals of kind=ROOT
  static Future _finalizeKindRoot(PostgreSQLConnection db, int proposalId, Map<String, dynamic> proposalRow, int winOption) async {
    //make a new document revision
    int docId = proposalRow['doc_id'];
    String dtext = proposalRow['dtext']; //new complete doc
    if (winOption == 0) {
      String docTitle = await MiscLib.queryScalar(db, 'select title from doc where id=${docId}', null);
