...
class DateLib {
  ...
  static String formatDateTime(DateTime dUtc, [String tzName=null]) {
    if (tzName == null)
      return _dateFormat.format(dUtc);
    DateTime formattable = utcToLocal(dUtc, tzName);
    return _dateFormat.format(formattable) + ' ' + _timeFormat.format(formattable);
  }
  ...
  static DateTime utcToLocal(DateTime dUtc, [String tzName = null]) {
    ...
    return new DateTime(dLocal.year, dLocal.month, dLocal.day, dLocal.hour, dLocal.minute);
  }
  ...
  static DateTime localToUtc(DateTime dLocal, [String tzName = null]) {
    ...
    Location l = getLocation(tzName);
    TZDateTime dUtc = new TZDateTime(l, dLocal.year, dLocal.month, dLocal.day, dLocal.hour, dLocal.minute).toUtc();
    return dUtc;
  }
  ...
  static String packUtcDateEntry(DateTime dUtc, String tzName) {
    if (tzName == null) return WLib.packDateEntry(dUtc.year, dUtc.month, dUtc.day, dUtc.hour, dUtc.minute);
    Location l = getLocation(tzName);
    TZDateTime dLocal = new TZDateTime.from(dUtc, l);
    return WLib.packDateEntry(dLocal.year, dLocal.month, dLocal.day, dLocal.hour, dLocal.minute);
  }
  ...
  static DateTime unpackConvertDateEntry(String packed, String tzName) {
    ...
    if (tzName != null) {
      ...
      return local.toUtc();
    } else { //timezone not known, treat input as UTC
